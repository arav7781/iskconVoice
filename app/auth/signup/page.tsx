  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: `${window.location.origin}/spiritual-journey`,
        },
      })
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/spiritual-journey`,
        },
      })
      if (error) throw error
    } catch (error: unknown) {
      ...
      <div className="w-full max-w-md relative z-10">
        <Card className="backdrop-blur-sm bg-white/90 border-orange-200 shadow-xl">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-gray-600">Begin your spiritual journey with us</CardDescription>
          </CardHeader>
          <CardContent>
